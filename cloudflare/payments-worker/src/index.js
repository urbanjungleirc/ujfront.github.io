...
async function sendVoucherEmail(env, opts) {
  const { to, customerName, voucherCode, value, expiryDate, typeName, emailSubject, termsConditions, giftFrom, giftTo, giftMessage } = opts;

  const subject = emailSubject || `Your Urban Jungle ${typeName}`;
  const html = renderVoucherEmail({ customerName, voucherCode, value, expiryDate, typeName, termsConditions, giftFrom, giftTo, giftMessage });
  ...
  <div class="voucher-box" style="background:#fff;border-radius:8px;box-shadow:0 2px 12px rgba(0,0,0,.1);overflow:hidden;">
    ...
    <div style="padding:24px;">
      ...
      <p style="margin:0 0 16px;font-size:15px;color:#333;">
        ${customerName ? `Hi ${escHtml(customerName)},<br><br>` : ''}
        Here is your Urban Jungle ${escHtml(typeName)}. Present this email at the front desk or scan the QR code below.
      </p>
        ...
        <table style="width:100%;border-top:1px solid #eee;padding-top:12px;margin-top:4px;">
          <tr>
            <td style="text-align:center;padding:4px;">
              <p style="margin:0 0 2px;font-size:11px;color:#999;text-transform:uppercase;">Value</p>
              <p style="margin:0;font-size:20px;font-weight:700;color:#333;">${formattedValue}</p>
            </td>
            <td style="text-align:center;padding:4px;border-left:1px solid #eee;">
              <p style="margin:0 0 2px;font-size:11px;color:#999;text-transform:uppercase;">Expires</p>
              <p style="margin:0;font-size:14px;font-weight:600;color:#333;">${formattedExpiry}</p>
            </td>
          </tr>
        </table>
      </div>

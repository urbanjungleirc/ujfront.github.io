...
function initializeTable() {
    currentTable = $("#results")
        ...
        .DataTable({
            ...
            dom: 'rt<"nav nav-fill my-1 mx-2" <"nav-item" i><"nav-item" p> >',
        });
...
function setupPageRotation() {
    ...
    if (tableInfo.pages > 1) {
        console.log(
            `Setting up auto page rotation: ${tableInfo.pages} pages total`
        );
        ...
        setInterval(function () {
            ...
            if (currentPageInfo.page == currentPageInfo.pages - 1) {
                ...
                console.log(
                    `Page rotation: Moving to page ${currentPageInfo.page + 2}`
                );
            }
        }, timePerPage);

        console.log(
            `Page will flip every ${timePerPage}ms (${timePerPage / 1000}s)`
        );
    } else {
...
function applyActiveFilter() {
    ...
    if (activeCategories.length > 0) {
        ...
        const searchPattern = activeCategories
...
function updateActiveCategories(scheduleData) {
    ...
    const categoriesChanged =
        JSON.stringify(newActiveCategories.sort()) !==
...
function updateActiveCategoriesDisplay() {
    ...
    if (display) {
        if (activeCategories.length > 0) {
            display.textContent = activeCategories
...
function updateCategoryDisplay() {
    ...
    if (display) {
        if (activeCategories.length > 0) {
            const categoryText = activeCategories
                .map((cat) => cat.charAt(0).toUpperCase() + cat.slice(1))
                .join(", ");

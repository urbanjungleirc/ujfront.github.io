    ...
    */
    ...
    function readSetting() {
        ...
        console.log(data);
        setting.data = data;
        findCurrentNext(); // Your custom function
        resetCategory();   // Your custom function
    
        // Cleanup: Remove the script tag and callback
        delete window.jsonpCallback;
        document.body.removeChild(script);
        };
    
        // Set the script URL with the callback query parameter
    ...
    function comparedSpeed(active, other1, other2, other3) {
        if (active) {
            ...
            if (isNaN(active)) {
                ...
                const number = $.fn.dataTable.render
                    .number(",", ".", 3)
                    .display(active);
                if (active == fastest) {
                    fclasses = "text-speed fw-bold";
    ...
    */
    ...
    function outputData() {
        ...
        maker(
            ...
            output,
            "text-secondary",
            `next up: ${setting.data.nextUp[0][2]}`
        );
        ...
        setting.data.schedule.forEach((category) => {
            ...
            if (category) {
                ...
                const val = maker(
                    ...
                    list,
                    "value",
                    `${category.name} from ${compStart} till ${compEnd}`
                );
            }
        });
    }
